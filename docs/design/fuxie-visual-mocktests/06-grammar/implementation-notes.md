# Implementation Notes — 06-grammar

## Module learning intent
Nắm rule ngữ pháp với ví dụ minh hoạ.

## Layout grid (desktop)
- Viewport reference 1440×900 px.
- Rule banner header + 2-column explain/example.
...
- Viewport 390×844 px.
- Rule header + accordion explain/example.
...
- Secondary palette module-specific: violet + paper white — tham chiếu vào Style_Master secondary palette tokens (sẽ chốt khi Style_Master Token registry hoàn thiện).
...
- Add (P0): module-specific component cho prop "grammar diagram" — định nghĩa biến thể trong Style_Master nếu reusable, hoặc local nếu module-only.
...
- Trạng thái đã chọn: **error state — sai pattern thường gặp với feedback chi tiết**.
- Lý do: trạng thái này là điểm đau / điểm mạnh đặc trưng nhất của module 06-grammar; đại diện được Module_Identity ngay cả khi flow chính không khả dụng (Requirement 5 AC 3).
...
- Module 06-grammar KHÔNG sao chép Mykonos (Greek-island visuals, Aegean palette, Cycladic architecture, white-blue domed buildings, Mediterranean village motifs) và KHÔNG sao chép Two Point Campus (characters, place names, themed props, campus parody buildings, mascot caricatures).

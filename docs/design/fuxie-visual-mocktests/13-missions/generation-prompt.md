# Generation Prompt — 13-missions

## Visual intent
Mission board with progress trail — directional, encouraging

## Module identity cues
Mission list + progress bar, palette emerald + soft cream, prop mission flag xác định module missions
...
Module: 13-missions. Visual intent: Mission board with progress trail — directional, encouraging. Identity cues: Mission list + progress bar, palette emerald + soft cream, prop mission flag xác định module missions.

Render three viewports as separate images:
- Mock_Desktop viewport 1440×900 px: Daily missions với 3 nhiệm vụ, navigation đầy đủ, vùng nội dung chính ≥ 60% chiều rộng viewport, đúng 1 CTA chính enabled, ít nhất 1 dấu hiệu Module_Identity (logo / tên module / icon đặc trưng "mission flag") trong vùng nhìn đầu.
...
- Mock_State: empty state — đã hoàn thành tất cả mission hôm nay. Module_Identity vẫn nhận diện được. Nếu là error state, phải có thông điệp lỗi đầy đủ + ≥ 1 lối thoát (retry/back/contact).

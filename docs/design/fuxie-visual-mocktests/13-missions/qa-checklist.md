# QA Checklist — 13-missions

## Learning intent (3s)
- Reviewer (chưa biết tên module) chọn đúng learning intent từ 18 lựa chọn trong ≤ 3000 ms?
- Score: 20 / 20
- Lý do (current vs target): PASS — mission board, 3 daily missions, progress bars, weekly trail, and primary CTA make the learning intent "Theo dõi nhiệm vụ ngày/tuần và CTA hoàn thành" readable within 3 seconds.
...
- Score: 15 / 15
- Lý do: PASS — mission flag, mission list/progress layout, daily/weekly objective framing, and emerald accent system distinguish this module from rewards, review, exam, and dashboard.

## Style master compliance
- Mọi token được dùng đều tham chiếu Style_Master, không khai báo token mới?
- Score: 14 / 15
- Lý do: PASS — Bright Sky palette, teal/emerald accents, rounded cards, raised shadows, and CTA hierarchy align with Style_Master. Minor deduction because static PNG generation uses illustrative shading that must be normalized into exact production tokens later.
...
- Score: 18 / 20
- Lý do: PASS — 390×844 mobile mock has no horizontal overflow, compact header, visible title, mission cards, weekly progress, and primary CTA. Minor deduction because the mobile screen is information-dense and needs final DOM copy/spacing validation.
...
- Score: 13 / 15
- Lý do: PASS — primary text and CTA contrast are visually strong across desktop/mobile/state. Minor deduction because some teal-on-soft-sky chips and small secondary labels should be re-measured on live DOM for WCAG.
...
- mock-state.png tồn tại, > 0 byte, mở đúng MỘT trạng thái phụ duy nhất đã chỉ định trong implementation-notes.md (trạng thái đã chọn: empty state — đã hoàn thành tất cả mission hôm nay)?
- Result: PASS — PNGs machine-verified after built-in `image_gen` render pass: mock-desktop.png 1440×900 / 2,002,335 bytes; mock-mobile.png 390×844 / 515,861 bytes; mock-state.png 1440×900 / 2,139,916 bytes. mock-state.png is the single all-missions-complete empty state; zero mock-state-*.png variants.
...
- Residual risk: Generated German microcopy, mission reward values, and teal/soft-sky contrast should be revalidated against final production copy and live DOM WCAG measurements during frontend implementation.

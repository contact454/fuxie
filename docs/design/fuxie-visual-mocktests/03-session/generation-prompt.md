# Generation Prompt — 03-session

## Visual intent
Calm focused study stage — single task, clear next step

## Module identity cues
Single stage center với step counter và play arrow CTA, palette teal + cool grey, prop play arrow xác định module session
...
Module: 03-session. Visual intent: Calm focused study stage — single task, clear next step. Identity cues: Single stage center với step counter và play arrow CTA, palette teal + cool grey, prop play arrow xác định module session.

Render three viewports as separate images:
- Mock_Desktop viewport 1440×900 px: Start today's session — step 1 of N, navigation đầy đủ, vùng nội dung chính ≥ 60% chiều rộng viewport, đúng 1 CTA chính enabled, ít nhất 1 dấu hiệu Module_Identity (logo / tên module / icon đặc trưng "play/start arrow") trong vùng nhìn đầu.
...
- Mock_State: success state — phiên học hoàn thành. Module_Identity vẫn nhận diện được. Nếu là error state, phải có thông điệp lỗi đầy đủ + ≥ 1 lối thoát (retry/back/contact).
...
## Reviewer + date
- Reviewer: Pack_Owner + QA_Owner delegated sign-off via Antigravity; Illustrator co-review adopted from QC report.
- Date (ISO 8601): 2026-05-20.
- Provenance Link: [.kiro/specs/fuxie-visual-mocktest-pack/codex-03-session-visual-qc-2026-05-20.md](../../../../.kiro/specs/fuxie-visual-mocktest-pack/codex-03-session-visual-qc-2026-05-20.md)

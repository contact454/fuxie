# Implementation Notes — 07-listening

## Module learning intent
Nghe đoạn audio chuẩn và bắt key information.

## Layout grid (desktop)
- Viewport reference 1440×900 px.
- Player top + transcript trái + question phải.
...
- Viewport 390×844 px.
- Player + collapsible transcript + question.
...
- Secondary palette module-specific: deep navy + soft yellow — tham chiếu vào Style_Master secondary palette tokens (sẽ chốt khi Style_Master Token registry hoàn thiện).
...
- Add (P0): module-specific component cho prop "headphone + waveform" — định nghĩa biến thể trong Style_Master nếu reusable, hoặc local nếu module-only.
...
- Trạng thái đã chọn: **loading state — đang tải audio**.
- Lý do: trạng thái này là điểm đau / điểm mạnh đặc trưng nhất của module 07-listening; đại diện được Module_Identity ngay cả khi flow chính không khả dụng (Requirement 5 AC 3).
...
- Module 07-listening KHÔNG sao chép Mykonos (Greek-island visuals, Aegean palette, Cycladic architecture, white-blue domed buildings, Mediterranean village motifs) và KHÔNG sao chép Two Point Campus (characters, place names, themed props, campus parody buildings, mascot caricatures).
...
- Pack_Owner ký xác nhận sau render thật: PASS — co-reviewed with Illustrator / 3D Mascot Artist on 2026-05-21; no forbidden IP references observed in the rendered mocks.

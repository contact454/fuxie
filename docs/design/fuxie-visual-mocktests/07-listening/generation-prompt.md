# Generation Prompt — 07-listening

## Visual intent
Radio booth focus moment — attentive listening, clear question framing

## Module identity cues
Waveform + transcript pane, palette deep navy + soft yellow, prop headphone + waveform xác định module listening
...
Module: 07-listening. Visual intent: Radio booth focus moment — attentive listening, clear question framing. Identity cues: Waveform + transcript pane, palette deep navy + soft yellow, prop headphone + waveform xác định module listening.

Render three viewports as separate images:
- Mock_Desktop viewport 1440×900 px: Listen and answer trong session, navigation đầy đủ, vùng nội dung chính ≥ 60% chiều rộng viewport, đúng 1 CTA chính enabled, ít nhất 1 dấu hiệu Module_Identity (logo / tên module / icon đặc trưng "headphone + waveform") trong vùng nhìn đầu.
...
- Mock_State: loading state — đang tải audio. Module_Identity vẫn nhận diện được. Nếu là error state, phải có thông điệp lỗi đầy đủ + ≥ 1 lối thoát (retry/back/contact).
...
- Actual render provenance: built-in `image_gen` via Codex `imagegen` skill / GPT image pipeline, user-approved override after CLI `gpt-image-2` key block. Source renders generated on 2026-05-21, then deterministically center-cropped/resized to the required PNG dimensions in place.

## Reviewer + date
- Reviewer: Illustrator / 3D Mascot Artist render pass via Codex built-in `image_gen`; Pack_Owner originality co-review + QA_Owner Visual_Target_Score sign-off via Codex QC.

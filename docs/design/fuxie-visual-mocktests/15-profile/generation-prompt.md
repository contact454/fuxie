# Generation Prompt — 15-profile

## Visual intent
Personal study room — reflective, goal-aware, calm

## Module identity cues
Avatar + goals card + tab navigation, palette mauve + ivory, prop avatar + goal flag xác định module profile
...
Module: 15-profile. Visual intent: Personal study room — reflective, goal-aware, calm. Identity cues: Avatar + goals card + tab navigation, palette mauve + ivory, prop avatar + goal flag xác định module profile.

Render three viewports as separate images:
- Mock_Desktop viewport 1440×900 px: My profile — overview tab, navigation đầy đủ, vùng nội dung chính ≥ 60% chiều rộng viewport, đúng 1 CTA chính enabled, ít nhất 1 dấu hiệu Module_Identity (logo / tên module / icon đặc trưng "avatar + goal flag") trong vùng nhìn đầu.
...
- Mock_State: success state — vừa cập nhật mục tiêu cá nhân. Module_Identity vẫn nhận diện được. Nếu là error state, phải có thông điệp lỗi đầy đủ + ≥ 1 lối thoát (retry/back/contact).

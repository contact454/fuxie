# Generation Prompt — 14-chat

## Visual intent
Chat cafe corner — warm conversation, attentive listening

## Module identity cues
Chat thread + composer, palette periwinkle + neutral grey, prop speech bubble xác định module chat
...
Module: 14-chat. Visual intent: Chat cafe corner — warm conversation, attentive listening. Identity cues: Chat thread + composer, palette periwinkle + neutral grey, prop speech bubble xác định module chat.

Render three viewports as separate images:
- Mock_Desktop viewport 1440×900 px: Ask Fuxie tutor với thread hiện tại, navigation đầy đủ, vùng nội dung chính ≥ 60% chiều rộng viewport, đúng 1 CTA chính enabled, ít nhất 1 dấu hiệu Module_Identity (logo / tên module / icon đặc trưng "speech bubble") trong vùng nhìn đầu.
...
- Mock_State: loading state — tutor đang trả lời (typing indicator). Module_Identity vẫn nhận diện được. Nếu là error state, phải có thông điệp lỗi đầy đủ + ≥ 1 lối thoát (retry/back/contact).

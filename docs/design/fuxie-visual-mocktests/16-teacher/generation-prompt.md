# Generation Prompt — 16-teacher

## Visual intent
Teacher staff room — operational clarity, student-aware

## Module identity cues
Roster table + assignment chip, palette slate + mustard, prop clipboard/roster xác định module teacher
...
Module: 16-teacher. Visual intent: Teacher staff room — operational clarity, student-aware. Identity cues: Roster table + assignment chip, palette slate + mustard, prop clipboard/roster xác định module teacher.

Render three viewports as separate images:
- Mock_Desktop viewport 1440×900 px: Class A2 — roster + this week assignment, navigation đầy đủ, vùng nội dung chính ≥ 60% chiều rộng viewport, đúng 1 CTA chính enabled, ít nhất 1 dấu hiệu Module_Identity (logo / tên module / icon đặc trưng "clipboard / roster") trong vùng nhìn đầu.
...
- Mock_State: error state — assignment quá hạn submission, hiển thị danh sách overdue + nudge action. Module_Identity vẫn nhận diện được. Nếu là error state, phải có thông điệp lỗi đầy đủ + ≥ 1 lối thoát (retry/back/contact).
...
- Actual render provenance: built-in `image_gen` via Codex imagegen skill / GPT image pipeline. Source renders generated on 2026-05-21, then deterministically resized to the required PNG dimensions in place.

## Reviewer + date
- Reviewer: QA_Owner (Codex QC 2026-05-21 authorized; Pack_Owner originality co-review complete; render pass by Illustrator / 3D Mascot Artist via Codex built-in `image_gen`).

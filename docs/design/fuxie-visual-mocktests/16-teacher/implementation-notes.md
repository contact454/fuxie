# Implementation Notes — 16-teacher

## Module learning intent
Giáo viên theo dõi lớp và giao bài.

## Layout grid (desktop)
- Viewport reference 1440×900 px.
- Class roster table trái + assignment panel phải.
...
- Viewport 390×844 px.
- Tabbed roster/assignment full-width.
...
- Secondary palette module-specific: slate + mustard — tham chiếu vào Style_Master secondary palette tokens (sẽ chốt khi Style_Master Token registry hoàn thiện).
...
- Add (P0): module-specific component cho prop "clipboard / roster" — định nghĩa biến thể trong Style_Master nếu reusable, hoặc local nếu module-only.
...
- Trạng thái đã chọn: **error state — assignment quá hạn submission, hiển thị danh sách overdue + nudge action**.
- Lý do: trạng thái này là điểm đau / điểm mạnh đặc trưng nhất của module 16-teacher; đại diện được Module_Identity ngay cả khi flow chính không khả dụng (Requirement 5 AC 3).
...
- Module 16-teacher KHÔNG sao chép Mykonos (Greek-island visuals, Aegean palette, Cycladic architecture, white-blue domed buildings, Mediterranean village motifs) và KHÔNG sao chép Two Point Campus (characters, place names, themed props, campus parody buildings, mascot caricatures).

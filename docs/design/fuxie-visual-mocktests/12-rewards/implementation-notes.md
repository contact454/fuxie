# Implementation Notes — 12-rewards

## Module learning intent
Hiển thị Fucoin / streak / badge đã đạt.

## Layout grid (desktop)
- Viewport reference 1440×900 px.
- Badge wall grid + Fucoin counter hero.
...
- Secondary palette module-specific: gold + cocoa — tham chiếu vào Style_Master secondary palette tokens (sẽ chốt khi Style_Master Token registry hoàn thiện).
...
- Add (P0): module-specific component cho prop "trophy / coin" — định nghĩa biến thể trong Style_Master nếu reusable, hoặc local nếu module-only.
...
- Trạng thái đã chọn: **success state — vừa unlock badge mới với reveal animation cue**.
- Lý do: trạng thái này là điểm đau / điểm mạnh đặc trưng nhất của module 12-rewards; đại diện được Module_Identity ngay cả khi flow chính không khả dụng (Requirement 5 AC 3).
...
- Module 12-rewards KHÔNG sao chép Mykonos (Greek-island visuals, Aegean palette, Cycladic architecture, white-blue domed buildings, Mediterranean village motifs) và KHÔNG sao chép Two Point Campus (characters, place names, themed props, campus parody buildings, mascot caricatures).

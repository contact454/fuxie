# Generation Prompt — 12-rewards

## Visual intent
Village reward hall — celebratory but honest, restrained

## Module identity cues
Badge wall + Fucoin counter, palette gold + cocoa, prop trophy/coin xác định module rewards
...
Module: 12-rewards. Visual intent: Village reward hall — celebratory but honest, restrained. Identity cues: Badge wall + Fucoin counter, palette gold + cocoa, prop trophy/coin xác định module rewards.

Render three viewports as separate images:
- Mock_Desktop viewport 1440×900 px: Your rewards sau khi vừa unlock, navigation đầy đủ, vùng nội dung chính ≥ 60% chiều rộng viewport, đúng 1 CTA chính enabled, ít nhất 1 dấu hiệu Module_Identity (logo / tên module / icon đặc trưng "trophy / coin") trong vùng nhìn đầu.
...
- Mock_State: success state — vừa unlock badge mới với reveal animation cue. Module_Identity vẫn nhận diện được. Nếu là error state, phải có thông điệp lỗi đầy đủ + ≥ 1 lối thoát (retry/back/contact).

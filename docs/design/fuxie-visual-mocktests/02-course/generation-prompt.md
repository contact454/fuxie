# Generation Prompt — 02-course

## Visual intent
Signpost path through CEFR levels — clear navigation, considered choice

## Module identity cues
Filter rail + course card grid với CEFR badge nổi bật, palette indigo + neutral, prop CEFR badge xác định module course
...
Module: 02-course. Visual intent: Signpost path through CEFR levels — clear navigation, considered choice. Identity cues: Filter rail + course card grid với CEFR badge nổi bật, palette indigo + neutral, prop CEFR badge xác định module course.

Render three viewports as separate images:
- Mock_Desktop viewport 1440×900 px: Course catalog với filter A1–B2, navigation đầy đủ, vùng nội dung chính ≥ 60% chiều rộng viewport, đúng 1 CTA chính enabled, ít nhất 1 dấu hiệu Module_Identity (logo / tên module / icon đặc trưng "CEFR badge") trong vùng nhìn đầu.
...
- Mock_State: loading state — đang tải catalog. Module_Identity vẫn nhận diện được. Nếu là error state, phải có thông điệp lỗi đầy đủ + ≥ 1 lối thoát (retry/back/contact).
...
## Reviewer + date
- Reviewer: Codex initial renderer (Pack_Owner + Illustrator co-review pending).

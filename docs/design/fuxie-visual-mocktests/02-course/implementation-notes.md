# Implementation Notes — 02-course

## Module learning intent
Khám phá lộ trình CEFR và chọn course.

## Layout grid (desktop)
- Viewport reference 1440×900 px.
- Filter rail trái + course card grid 3 cột.
...
- Secondary palette module-specific: indigo + neutral grey — tham chiếu vào Style_Master secondary palette tokens (sẽ chốt khi Style_Master Token registry hoàn thiện).
...
- Add (P0): module-specific component cho prop "CEFR badge" — định nghĩa biến thể trong Style_Master nếu reusable, hoặc local nếu module-only.
...
- Trạng thái đã chọn: **loading state — đang tải catalog**.
- Lý do: trạng thái này là điểm đau / điểm mạnh đặc trưng nhất của module 02-course; đại diện được Module_Identity ngay cả khi flow chính không khả dụng (Requirement 5 AC 3).
...
- Module 02-course KHÔNG sao chép Mykonos (Greek-island visuals, Aegean palette, Cycladic architecture, white-blue domed buildings, Mediterranean village motifs) và KHÔNG sao chép Two Point Campus (characters, place names, themed props, campus parody buildings, mascot caricatures).

# Generation Prompt — 01-dashboard

## Visual intent
Cozy German-village square morning briefing — calm, oriented, hopeful

## Module identity cues
Hero "today" panel với progress ring + 3 KPI tile, palette sky-blue + warm sand, prop progress ring xác định module dashboard ngay từ vùng nhìn đầu
...
Module: 01-dashboard. Visual intent: Cozy German-village square morning briefing — calm, oriented, hopeful. Identity cues: Hero "today" panel với progress ring + 3 KPI tile, palette sky-blue + warm sand, prop progress ring xác định module dashboard ngay từ vùng nhìn đầu.

Render three viewports as separate images:
- Mock_Desktop viewport 1440×900 px: Today view với streak + next session CTA, navigation đầy đủ, vùng nội dung chính ≥ 60% chiều rộng viewport, đúng 1 CTA chính enabled, ít nhất 1 dấu hiệu Module_Identity (logo / tên module / icon đặc trưng "progress ring") trong vùng nhìn đầu.
...
- Mock_State: empty state — chưa có session hôm nay. Module_Identity vẫn nhận diện được. Nếu là error state, phải có thông điệp lỗi đầy đủ + ≥ 1 lối thoát (retry/back/contact).
...
(Provenance refresh rule — Requirement 12 AC 6: file này được cập nhật trong cùng change set với formal sign-off; render PNG đã được external pipeline ghi đè placeholder ở change set trước; QC report tại [`codex-01-dashboard-visual-qc-2026-05-17.md`](../../../.kiro/specs/fuxie-visual-mocktest-pack/codex-01-dashboard-visual-qc-2026-05-17.md).)

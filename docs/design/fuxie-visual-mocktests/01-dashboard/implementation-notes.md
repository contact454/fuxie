# Implementation Notes — 01-dashboard

## Module learning intent
Tổng quan tiến trình học hôm nay và CTA tiếp theo.

## Layout grid (desktop)
- Viewport reference 1440×900 px.
- Hero "today" + 3 KPI tile row (3-column).
...
- Secondary palette module-specific: sky-blue + warm sand — tham chiếu vào Style_Master secondary palette tokens (sẽ chốt khi Style_Master Token registry hoàn thiện).
...
- Add (P0): module-specific component cho prop "progress ring" — định nghĩa biến thể trong Style_Master nếu reusable, hoặc local nếu module-only.
...
- Trạng thái đã chọn: **empty state — chưa có session hôm nay**.
- Lý do: trạng thái này là điểm đau / điểm mạnh đặc trưng nhất của module 01-dashboard; đại diện được Module_Identity ngay cả khi flow chính không khả dụng (Requirement 5 AC 3).
...
- Module 01-dashboard KHÔNG sao chép Mykonos (Greek-island visuals, Aegean palette, Cycladic architecture, white-blue domed buildings, Mediterranean village motifs) và KHÔNG sao chép Two Point Campus (characters, place names, themed props, campus parody buildings, mascot caricatures).

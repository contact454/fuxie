# Generation Prompt — 10-writing

## Visual intent
Post office writing counter — focused composition, structural support

## Module identity cues
Writing canvas + hint chips, palette plum + soft sand, prop pen/cursor xác định module writing
...
Module: 10-writing. Visual intent: Post office writing counter — focused composition, structural support. Identity cues: Writing canvas + hint chips, palette plum + soft sand, prop pen/cursor xác định module writing.

Render three viewports as separate images:
- Mock_Desktop viewport 1440×900 px: Write your answer với editor active, navigation đầy đủ, vùng nội dung chính ≥ 60% chiều rộng viewport, đúng 1 CTA chính enabled, ít nhất 1 dấu hiệu Module_Identity (logo / tên module / icon đặc trưng "pen / cursor") trong vùng nhìn đầu.
...
- Mock_State: error state — thiếu yêu cầu cấu trúc, feedback inline. Module_Identity vẫn nhận diện được. Nếu là error state, phải có thông điệp lỗi đầy đủ + ≥ 1 lối thoát (retry/back/contact).

# Implementation Notes — 10-writing

## Module learning intent
Viết câu / đoạn văn với hint cấu trúc.

## Layout grid (desktop)
- Viewport reference 1440×900 px.
- Editor center + hint rail phải.
...
- Viewport 390×844 px.
- Editor full-width + hint sheet bottom.
...
- Secondary palette module-specific: plum + soft sand — tham chiếu vào Style_Master secondary palette tokens (sẽ chốt khi Style_Master Token registry hoàn thiện).
...
- Add (P0): module-specific component cho prop "pen / cursor" — định nghĩa biến thể trong Style_Master nếu reusable, hoặc local nếu module-only.
...
- Trạng thái đã chọn: **error state — thiếu yêu cầu cấu trúc, feedback inline**.
- Lý do: trạng thái này là điểm đau / điểm mạnh đặc trưng nhất của module 10-writing; đại diện được Module_Identity ngay cả khi flow chính không khả dụng (Requirement 5 AC 3).
...
- Module 10-writing KHÔNG sao chép Mykonos (Greek-island visuals, Aegean palette, Cycladic architecture, white-blue domed buildings, Mediterranean village motifs) và KHÔNG sao chép Two Point Campus (characters, place names, themed props, campus parody buildings, mascot caricatures).

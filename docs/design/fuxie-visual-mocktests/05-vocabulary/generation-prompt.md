# Generation Prompt — 05-vocabulary

## Visual intent
Collection book scene — joyful word discovery, supportive feedback

## Module identity cues
Word card lớn + flashcard motion hint, palette coral + warm cream, prop flashcard xác định module vocabulary
...
Module: 05-vocabulary. Visual intent: Collection book scene — joyful word discovery, supportive feedback. Identity cues: Word card lớn + flashcard motion hint, palette coral + warm cream, prop flashcard xác định module vocabulary.

Render three viewports as separate images:
- Mock_Desktop viewport 1440×900 px: Learn 10 new words với card front, navigation đầy đủ, vùng nội dung chính ≥ 60% chiều rộng viewport, đúng 1 CTA chính enabled, ít nhất 1 dấu hiệu Module_Identity (logo / tên module / icon đặc trưng "flashcard") trong vùng nhìn đầu.
...
- Mock_State: success state — đã thuộc 10 từ. Module_Identity vẫn nhận diện được. Nếu là error state, phải có thông điệp lỗi đầy đủ + ≥ 1 lối thoát (retry/back/contact).
...
## Reviewer + date
- Reviewer: Codex imagegen skill (gpt-image-2.0 render) — pending Pack_Owner + Illustrator / 3D Mascot Artist originality co-review and QA_Owner scoring.

# Generation Prompt — 09-reading

## Visual intent
Library reading nook — quiet focus, comprehension cue

## Module identity cues
Reading column rộng + side question, palette sage + ivory, prop open book xác định module reading
...
Module: 09-reading. Visual intent: Library reading nook — quiet focus, comprehension cue. Identity cues: Reading column rộng + side question, palette sage + ivory, prop open book xác định module reading.

Render three viewports as separate images:
- Mock_Desktop viewport 1440×900 px: Read and answer với passage hiện tại, navigation đầy đủ, vùng nội dung chính ≥ 60% chiều rộng viewport, đúng 1 CTA chính enabled, ít nhất 1 dấu hiệu Module_Identity (logo / tên module / icon đặc trưng "open book") trong vùng nhìn đầu.
...
- Mock_State: success state — đạt comprehension threshold. Module_Identity vẫn nhận diện được. Nếu là error state, phải có thông điệp lỗi đầy đủ + ≥ 1 lối thoát (retry/back/contact).

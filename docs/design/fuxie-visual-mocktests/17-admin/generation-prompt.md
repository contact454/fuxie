# Generation Prompt — 17-admin

## Visual intent
Admin operations console — precise, dense, keyboard-friendly

## Module identity cues
Data table + filter rail + topbar, palette charcoal + cyan accent, prop gear/dashboard tile xác định module admin
...
Module: 17-admin. Visual intent: Admin operations console — precise, dense, keyboard-friendly. Identity cues: Data table + filter rail + topbar, palette charcoal + cyan accent, prop gear/dashboard tile xác định module admin.

Render three viewports as separate images:
- Mock_Desktop viewport 1440×900 px: Users management với filter active, navigation đầy đủ, vùng nội dung chính ≥ 60% chiều rộng viewport, đúng 1 CTA chính enabled, ít nhất 1 dấu hiệu Module_Identity (logo / tên module / icon đặc trưng "gear / dashboard tile") trong vùng nhìn đầu.
...
- Mock_State: empty state — filter không trả kết quả, suggest reset. Module_Identity vẫn nhận diện được. Nếu là error state, phải có thông điệp lỗi đầy đủ + ≥ 1 lối thoát (retry/back/contact).

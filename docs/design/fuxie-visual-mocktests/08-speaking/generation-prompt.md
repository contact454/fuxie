# Generation Prompt — 08-speaking

## Visual intent
Speaking stage with warm encouraging feedback — confident, kindly corrective

## Module identity cues
Mic-centric layout + waveform feedback, palette rose + cool teal, prop microphone xác định module speaking
...
Module: 08-speaking. Visual intent: Speaking stage with warm encouraging feedback — confident, kindly corrective. Identity cues: Mic-centric layout + waveform feedback, palette rose + cool teal, prop microphone xác định module speaking.

Render three viewports as separate images:
- Mock_Desktop viewport 1440×900 px: Speak the sentence với scoring meter, navigation đầy đủ, vùng nội dung chính ≥ 60% chiều rộng viewport, đúng 1 CTA chính enabled, ít nhất 1 dấu hiệu Module_Identity (logo / tên module / icon đặc trưng "microphone") trong vùng nhìn đầu.
...
- Mock_State: error state — pronunciation lệch, hiển thị target âm + retry. Module_Identity vẫn nhận diện được. Nếu là error state, phải có thông điệp lỗi đầy đủ + ≥ 1 lối thoát (retry/back/contact).
...
- Actual render provenance: built-in `image_gen` via Codex `imagegen` skill / GPT image pipeline. Source renders generated on 2026-05-21, then deterministically center-cropped/resized to the required PNG dimensions in place.

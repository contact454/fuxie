# Implementation Notes — 08-speaking

## Module learning intent
Luyện phát âm với phản hồi pronunciation.

## Layout grid (desktop)
- Viewport reference 1440×900 px.
- Center mic large + feedback panel cạnh.
...
- Viewport 390×844 px.
- Mic full-bleed bottom + feedback above.
...
- Secondary palette module-specific: rose + cool teal — tham chiếu vào Style_Master secondary palette tokens (sẽ chốt khi Style_Master Token registry hoàn thiện).
...
- Add (P0): module-specific component cho prop "microphone" — định nghĩa biến thể trong Style_Master nếu reusable, hoặc local nếu module-only.
...
- Trạng thái đã chọn: **error state — pronunciation lệch, hiển thị target âm + retry**.
- Lý do: trạng thái này là điểm đau / điểm mạnh đặc trưng nhất của module 08-speaking; đại diện được Module_Identity ngay cả khi flow chính không khả dụng (Requirement 5 AC 3).
...
- Module 08-speaking KHÔNG sao chép Mykonos (Greek-island visuals, Aegean palette, Cycladic architecture, white-blue domed buildings, Mediterranean village motifs) và KHÔNG sao chép Two Point Campus (characters, place names, themed props, campus parody buildings, mascot caricatures).

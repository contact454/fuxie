...
- `scripts/content-d7-signoff-sweep.ts`: D7 register đã tạo từ board + manifest + sample packs, bao phủ 36/36 cell, 120 general review inputs, và 1.011 Vocabulary D7 review rows; 0 sample file thiếu.
...
- Vocabulary D7 advisory: 369/369 files and 10,461 entries checked; 239 files carry D7 remediation notes for objective schema, lexeme, semantic, and present-conjugation blockers. No `auto_generated_needs_spot_check` conjugations remain; 12 A1 month nouns are article/plural-aligned with Duden, all 17 article/pluralia P1 rows have explicit source-backed policy, 185 C1 plus 74 C2 plural morphology rows are Duden/direct-headword backed, 3 C1 plus 32 C2 rows are canonicalized from matching internal-corpus lexemes, senses, and morphology, and 172 C2 nominalized-infinitive/abstract-compound rows are regular-rule singular-only; `vocabulary-d7-review-pack.{json,md,csv}` now queues 1.011 rows (146 P1) for loanword policy, plural morphology, example fit, and final native signoff.

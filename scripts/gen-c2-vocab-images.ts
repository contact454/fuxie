/**
 * C2 Vocabulary Image Generator + JSON Updater
 * Reads all C2 theme JSON files, generates images, and updates JSON with imageUrl
 * 
 * Uses Gemini API with fallback chain:
 *   gemini-3.1-flash-image-preview → gemini-2.5-flash-image
 *
 * Usage:
 *   npx tsx scripts/gen-c2-vocab-images.ts                # all themes
 *   npx tsx scripts/gen-c2-vocab-images.ts --theme=01     # specific theme
 *   npx tsx scripts/gen-c2-vocab-images.ts --start=06     # start from theme 06
 *   npx tsx scripts/gen-c2-vocab-images.ts --dry-run      # just list words
 */
...
function buildPrompt(word: string, meaning: string, wordType: string): string {
    ...
    } else if (wordType === 'PHRASE' || wordType === 'PARTIKEL') {
...
async function main() {
    console.log('🦊 C2 Vocabulary Image Generator + JSON Updater')
    console.log('================================================')

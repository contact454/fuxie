...
export function applyListeningRegen(text: string, patch: ListeningRegenPatch): ApplyResult {
  ...
  parsed.transcript.status = 'complete'
  parsed.transcript.audio_restub = 'pending'
  parsed.transcript.note =
    'Transcript regenerated by content-listening-regeneration (release-candidate script); MP3 pending re-record (Audio_Restubbing).'
  ...
  if (parsed.cefrAudit && typeof parsed.cefrAudit === 'object') {
    ...
    parsed.cefrAudit.notes =
      'Regenerated listening transcript + questions, signed off by AI German Academic Lead (Kiro); MP3 pending re-record (Audio_Restubbing); pending native-speaker spot-check.'
  }
